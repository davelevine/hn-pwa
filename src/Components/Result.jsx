...
function ResultTitle({ children: title, domain, url }) {
  return (
    <div className="flex flex-row flex-wrap items-center">
      <a className="mr-2 font-regular" href={domain ? url : url.replace("item?id=", "/item/")} target="_blank" rel="nofollow noopener noreferrer">
        {title}
      </a>
...
function ResultMetaData({ index, score }) {
  return (
    <div className="flex flex-row justify-between sm:items-center sm:justify-start sm:w-24">
      ...
      <div className="order-1 pr-2 text-lg font-medium sm:order-2 sm:pl-3 text-orange">{score}</div>
    </div>
  );
}
